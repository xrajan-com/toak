...
void main() {
  ...
  test('first run is quiet only for lounge/table music', () async {
    final AppSettingsService settings = AppSettingsService();
    await settings.load();

    expect(settings.loungeSoundsEnabled, isFalse,
        reason: 'ambient lounge/table music must not play uninvited');
    expect(settings.soundEffectsEnabled, isTrue,
        reason: 'sound effects carry information and stay on in every game');
